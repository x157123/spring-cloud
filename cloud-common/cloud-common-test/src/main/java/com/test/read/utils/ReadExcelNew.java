...
public class ReadExcelNew {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ReadExcelNew.class);
    ...
    public static List<DatabaseParam> readExcel(String filePath) {
        Workbook workbook = null;
        ...
        try {
            ...
            // 读取excel中的数据
            List<DatabaseParam> resultDataList = parseExcel(workbook);
     ...
     * @return 解析结果
     */
    private static List<DatabaseParam> parseExcel(Workbook workbook) {
        // 解析sheet
        for (int sheetNum = 0; sheetNum < workbook.getNumberOfSheets(); sheetNum++) {
            List<DatabaseParam> resultDataList = new ArrayList<>();
            //只读取第一个页签
            ...
            String tableName = convertCellValueToString(firstRow.getCell(0));
            ...
            for (int rowNum = rowStart; rowNum < rowEnd; rowNum++) {
                ...
                DatabaseParam resultData = convertRowToData(row);
                if (null == resultData) {
            ...
            //打印 创见
            {
                ...
                boolean b = false;
                for (DatabaseParam databaseParam : resultDataList) {
                    if (b) {
                        System.out.println(",");
                    } else {
                        b = true;
                ...
                System.out.println();
                System.out.println(") COMMENT='" + workbookSheetName + "';");
            }
            //打印 建表查询语句
            {
                ...
                for (DatabaseParam databaseParam : resultDataList) {
                    ...
                    System.out.print("  " + databaseParam.getName());
                    System.out.print(" as " + databaseParam.getName());
                }
                System.out.println("");
                System.out.println("from " + tableName.replaceAll("yb_",""));
     ...
     */
    private static DatabaseParam convertRowToData(Row row) {
        DatabaseParam resultData = new DatabaseParam();
        Cell cell;
        int cellNum = 0;
        // 获取姓名
        ...
        // 长度
        cell = row.getCell(cellNum++);
        String length = convertCellValueToString(cell);
        resultData.setLength(Integer.parseInt(length));
        // 是否必填
        cell = row.getCell(cellNum++);
        // 备注
        cell = row.getCell(cellNum++);
