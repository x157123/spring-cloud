 ...
 * 读取 Excel
 */
public class ReadExcelMapData {

    private static final Logger logger = LoggerFactory.getLogger(ReadExcelMapData.class);
...
//            只读取第一个页签
        for (int sheetNum = 0; sheetNum < 1; sheetNum++) {
            ...
            for (int rowNum = rowStart; rowNum < rowEnd; rowNum++) {
                ...
                JSONObject map = convertRowToData(row);
                jsonArray.add(map);
            }
     ...
     */
    private static JSONObject convertRowToData(Row row) {
        ...
        double[] doub = MapCoordinateConvert.gcj2WGS(Double.parseDouble(map.getString("lat")), Double.parseDouble(map.getString("lon")));
