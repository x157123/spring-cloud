...
public class Pinyin4jUtils {
    ...
    public static void main(String[] args) {
        // 中文转成全拼大写测试，并用空白分割打印
        String[] strings = getFullPinyin("东禅", true);
        // 使用String.join来输出，如果你用的JDK比较落后，那么就迭代每个string，拼接出来
        System.out.println(String.join("", strings)); // PIN YIN CE SHI
        // 中文转成全拼大写测试，并用空白分割打印
        String[] stringss = getFullPinyin("东蝉", true);
        // 使用String.join来输出，如果你用的JDK比较落后，那么就迭代每个string，拼接出来
        System.out.println(String.join("", stringss)); // PIN YIN CE SHI
     ...
     * @param source 字符
     */
    public static String getFullPinyinStr(String source) {
        String[] str = getFullPinyin(source,Boolean.TRUE);
