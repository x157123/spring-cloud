...
public class ExcelWriter {
    ...
    private static void convertDataToRow(OrgCompare data, Row row, CellStyle roseStyle, CellStyle greenStyle, CellStyle blueStyle) {
         ...
         * 村 社区代码
         */
        OrgAll grid = data.getGrid();
        cell = row.createCell(cellNum++);
        setCell(cell, grid, roseStyle, greenStyle, blueStyle);
    }
        ...
        if(village!=null) {
            if (village.getCom() <= 1) {
                ...
                if (village.getType()) {
                    ...
                    OrgAll og = village.getFatherOrgAll();
                    if(village.getDealWith()==null || village.getDealWith()==false) {
                        if (og == null) {
                            OrgAll go = new OrgAll("成都市");
                            go.setNewId("500");
                            village.setFatherOrgAll(go);
                        }
                        Map<String, Long> map;
                        if (village.getOrgType() == null) {
                            System.out.println("错误");
                        }
                        System.out.println("组织缺少:" + village.getOrgName() + "--" + og.getNewId() + "----" + og.getOrgFullName());
//                        if (og.getNewId() == null){
//                            System.out.println("组织机构为null");
//                            Map<String, Long> maps;
//                            OrgAll ogs = og.getFatherOrgAll();
//                            if (og.getOrgType().equals("02")) {
//                                maps = OrtAdd.addZnOrg(Long.parseLong(ogs.getNewId()), og.getOrgName(), og.getMaxDept());
//                            } else {
//                                maps = OrtAdd.addXzOrg(Long.parseLong(ogs.getNewId()), og.getOrgName(), og.getMaxDept());
//                            }
//                            ogs.setMaxDept(maps.get("dept"));
//                            og.setNewId(maps.get("id").toString());
//                        }
//                        if (village.getOrgType().equals("02")) {
//                            map = OrtAdd.addZnOrg(Long.parseLong(og.getNewId()), village.getOrgName(), village.getMaxDept());
//                        } else {
//                            map = OrtAdd.addXzOrg(Long.parseLong(og.getNewId()), village.getOrgName(), village.getMaxDept());
//                        }
//                        og.setMaxDept(map.get("dept"));
//                        village.setNewId(map.get("id").toString());
//                        village.setDealWith(Boolean.TRUE);
                    }
                } else {
                ...
                if (village.getNewOrgName() != null && village.getNewOrgName().length() > 0) {
                    ...
                    if(village.getDealWith()==null || village.getDealWith()==false){
                        village.setDealWith(Boolean.TRUE);
//                        OrtAdd.updateOrgName(Long.parseLong(village.getNewId()),village.getOrgName());
                        System.out.println("组织机构名称不一致:" + village.getOrgName() + "--" + village.getNewOrgName() + "----" + village.getNewId());
                    }
                    //修改数据
